...
export default function BeautyAndHealthData() {
  ...
  const BeautyAndHealthDataData = async (page) => {
    setLoad(true);
    try {
      const dress = await axios.get(
        `https://63c705d44ebaa80285526612.mockapi.io/makeup?page=${page}&limit=12`
      );
  ...
  const HandleFilter = async (val) => {
    ...
    try {
      const dress = await axios.get(
        `https://63c705d44ebaa80285526612.mockapi.io/makeup?search=${val}`
      );
      setData(dress.data);
      setLoad(false);
    } catch (error) {
      ...
      <Box w={"87%"} m={"auto"} mt={{ base: "10px", md: "-25px" }}>
        <Stack>
          <Stack spacing={8} direction="row">
            <Box p={{ base: 0, md: 5 }}>
              <Heading fontSize="xl">Beauty and Health</Heading>
              <Text mt={4}>
        ...
        >
          ...
          <Box ml={"20px"}>
            ...
            >
              {load
                ? arr.map((item) => {
                    return <LoadingScreen key={item + 219540} />;
                  })
                : data.map((item, i) => {
                    return (
                      <ProductCards
